...
var installBuiltins func(dir string, store SessionStore) []Tool

// RegisterBuiltins installs the built-in tool factory. It is called from the
// tools package's init.
func RegisterBuiltins(fn func(dir string, store SessionStore) []Tool) {
	installBuiltins = fn
}

// builtinTools installs the built-in tools for a working directory.
func builtinTools(dir string, store SessionStore) []Tool {
	if installBuiltins == nil {
		return nil
	}
	return installBuiltins(dir, store)
}
